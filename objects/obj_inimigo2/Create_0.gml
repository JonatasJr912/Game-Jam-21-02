...
fantasma_quando_normal = false;   // inimigo2 = fantasma no invertido
morte_skeleton_tipo    = "parede"; // ao virar esqueleto no normal em cima de "parede", ele morre (simétrico)
    ...
    if (!chegou)
    {
        ...
        if (point_distance(x, y, tx, ty) <= eps)
        {
            ...
            if(!tocou_som)
            {
                 ...
                 snd_movimento_h = audio_play_sound(sfx_movimento, 1, false);
            }
        }
    }
    else
